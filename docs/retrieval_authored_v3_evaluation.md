...
H001's, H012's, and H013's held-out decomposition turns, each reached the provider boundary once but produced no response
...
bytes, 38,205 input-token upper bound, and 4,000 maximum output tokens). Their cumulative
`$3.334184375` reserve lowers the dynamic tracked ceiling to `$3.665815625`.
